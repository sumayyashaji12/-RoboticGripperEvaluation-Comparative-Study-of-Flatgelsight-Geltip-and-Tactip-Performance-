...
class Dummy_Grasp_Routine_Wrapper:
    ...
    def R_shake_gripper(self, amp=0.3):
    ...
    def R_grasp_object(self):
        self.advance_time(1)
        self.R_open_gripper()
        self.R_force_move_to_xyz(dst_xyz=[0.0,0.0,0.15])
...
if __name__ == "__main__":
    ...
    # Get the stable position as before
...
# Set the orientation to an upright position (you may need to adjust these values)
    # upright_orn = sim.p.getQuaternionFromEuler([  0,  np.pi / 2,   np.pi/2])
  # Example: upright orientation as a quaternion
    ...
    # Get the new position and orientation after resetting
    #new_pos, new_orn = sim.p.getBasePositionAndOrientation(bodyUniqueId=sim.object_id)

# Print the new orientation to confirm
    #print("New Position:", new_pos)
    #print("New Orientation:", new_orn)
